...
fccanalysis run analysis_2/bafb/python/analysis_stage1.py "${ARGS[@]}" --output "$OUT1"

echo ">> [run] Running stage-2 on: $OUT1"
fccanalysis run analysis_2/bafb/python/analysis_stage2.py "${NTHREADS_FLAG[@]}" --input "$OUT1"

echo ">> [run] Making plots (standalone script)"
python3 analysis_2/bafb/python/analysis_plots.py
